...
import os
from AFGProDistList import get_prov_dist_dict, get_check_province
year = input('Please enter year for analysis...')
months = ['January', 'February', 'March', 'April','May']
url_temp = 'https://en.wikipedia.org/wiki/List_of_terrorist_incidents_in_month_2019'
for month in months:
    print (month)
...
#Extract tr (rows) from table
    ...
    plt.title(month+ " 2019 Terrorist Incidents Around the worlds")
    ...
    for location in all_war_location:
        text = location.split()
        for item in text:
            if item in prov_list:
                last_prov_list[item] = last_prov_list.get(item,0)+1
            else:
                for key,district in prov_dict_list.items():
                    if item in district:
                        last_prov_list[key] = last_prov_list.get(key,0)+1
                    elif location in district:
                        last_prov_list[key] = last_prov_list.get(key, 0) + 1

    path = 'targetprovinces/' + year
    print(last_prov_list)
    if not os.path.exists(path):
        os.makedirs((path))
        file_path = path + '/'+ month + '.txt'
        if not os.path.exists(file_path):
            dc.write_list(last_prov_list,file_path )
    else:
        file_path = path + '/' + month + '.txt'
        if not os.path.exists(file_path):
            dc.write_list(last_prov_list, file_path)
